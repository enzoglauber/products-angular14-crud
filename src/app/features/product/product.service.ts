...
export class ProductService {
  ...
  private _filter = new BehaviorSubject<ProductFilter>({
    code: '',
    category: '',
  });
  ...
  getAll = () => {
    ...
    return this.http
      .get<Product[]>(`${environment.bff}/v1/products`, { params })
      .pipe(map(this.nextProducts));
  };
  ...
  save = (entity: Partial<Product>): Observable<Product> => {
    if (entity.id) {
      return this.http
        .put<Product>(`${environment.bff}/v1/products`, entity)
        .pipe(map(this.success));
    } else {
      return this.http
        .post<Product>(`${environment.bff}/v1/products`, entity)
        .pipe(map(this.success));
    }
  };
  ...
  delete = (entity: Partial<Product>): Observable<Partial<Product> | void> => {
    ...
    return this.http.delete<Product>(url).pipe(
      tap(() => {
        this.snackBar.open('Removed successfully!', undefined, {
          duration: 3000,
        });
  ...
  check = (entity: Pick<Product, 'code'>): Observable<{ exist: boolean }> => {
    return this.http.post<{ exist: boolean }>(
      `${environment.bff}/v1/products/check`,
      entity
    );
  };

  get all$(): Observable<Product[]> {
    return this.filter$.pipe(switchMap(this.getAll));
  }
